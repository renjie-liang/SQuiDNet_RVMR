...
def eval_by_task_type(moment_predictions, video2idx, ground_truth,
    ...
    # prediction length per positive and negative 
    pred_len_per_pos = np.zeros((40,1))
    pred_len_per_neg = np.zeros((40,1))
    max_num = 0
    for k, gt_item in tqdm(gt_by_desc_id.items(), desc="Loop over moments", leave=False):
        ...
        if len(gt_item["ts"]) >= 4:  # didemo, fro all 3 splits, at least 4 ts for each, < 0.5% has more than 4.
            ...
            for iou_thd in iou_thds:
                ...
                iou_thd_corrects_columns.append(iou_corrects[:, None])
                if iou_thd == 0.7:
                    if True in iou_corrects[:, None]:
                        go = int(num/5)
                        pred_len_per_pos[go] += 1
                    else:
                        go = int(num/5)
                        pred_len_per_neg[go] += 1
    ...
    pred_len = pred_len_per_pos + pred_len_per_neg + 0.0001
    pred_tot = pred_len_per_pos/pred_len
